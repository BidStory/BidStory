...
function convertSQLiteToJSON(sqliteFilePath, outputJsonPath) {
  ...
  db.serialize(() => {
    db.all("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'", (err, tables) => {
      if (err) return console.error("خطأ في قراءة أسماء الجداول:", err.message);
      ...
      tables.forEach((table) => {
        ...
        db.all(`SELECT * FROM ${tableName}`, (err, rows) => {
          if (err) return console.error(`خطأ في قراءة جدول ${tableName}:`, err.message);

          databaseJson[tableName] = rows;

          remaining--;
          if (remaining === 0) {
            fs.writeFileSync(outputJsonPath, JSON.stringify(databaseJson, null, 2), 'utf8');
